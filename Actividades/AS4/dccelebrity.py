...
class DCCelebrity:
    ...
    def crear_red(self, info_usuarios):
        ...
        for usuario in usuarios:
            self.lista_ligada.insertar_amigo_secreto(usuario, usuarios_recorridos)
            usuarios_recorridos += 1

        # Crear arbol
        nodos_fama = [NodoFama(usuario) for _, usuario in dict_usuarios.items()]
        nodos_ordenados = sorted(
            nodos_fama, key=lambda x: ORDEN_FAMA.index(x.usuario.fama)
        )
        self.arbol.crear_arbol(nodos_ordenados)

        # Crear nodos grafo
        self.nodos_grafo = {
            nombre: NodoGrafo(dict_usuarios[nombre]) for nombre in info_usuarios.keys()
        }
        # Crear amistades
        for nombre, atributos in info_usuarios.items():
            amistades = [
    ...
    def run(self):
        ...
        while running:
            if usuario is None:
                ...
                nombre = input("Introduce tu nombre para ingresar, " "o 0 para salir: ")
                if nombre == "0":
                    running = False
            ...
            if accion == "Volver":
                ...
                print(f"Usuario {usuario.nombre} " "tiene las siguientes amistades:")
                ...
                self.print_usuarios(
                    self.nodos_grafo.values(), ignorar_nombre=usuario.nombre
                )
                nombre = input("Introduce el nombre del " "usuario a agregar: ")
                ...
                nombre = input(
                    "Introduce el nombre de la " "amistad que queires eliminar: "
                )
                if nombre in self.nodos_grafo and nombre != usuario.nombre:
    ...
    def print_usuarios(nodos_grafos, ignorar_nombre=""):
        nodos_filtrados = filter(
            lambda x: x.usuario.nombre != ignorar_nombre, nodos_grafos
        )
        new_column = False
        print(
        ...
        for nodo in nodos_filtrados:
            ...
            print(
                f"* {nodo.usuario.nombre: ^32s}|" f" {nodo.usuario.correo: ^40s} *",
                end=end,
            )
    ...
    def acciones_validas(famoso: bool):
        ...
        return {str(num): accion for num, accion in enumerate(acciones, start=0)}
